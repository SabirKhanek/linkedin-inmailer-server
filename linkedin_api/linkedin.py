...
class Linkedin(object):
    ...
    def get_sales_profile(self, profile_id, evade=default_evade) -> dict:
        ...
        """
        evade()
        self.logger.debug("Sending request to fetch sales profile urn")
        
        self.verify_sales_subscription()

        decorations = f'decoration=%28entityUrn%2CprofilePictureDisplayImage%2CprofileBackgroundPicture%2CrelatedColleagueCompanyId%2CblockThirdPartyDataSharing%2CnoteCount%2Ceducations*%28degree%2CeduId%2CendedOn%2CschoolName%2CstartedOn%2CfieldsOfStudy*%2CrichMedia*%2Cschool~fs_salesSchool%28entityUrn%2ClogoId%2Cname%2Curl%2CschoolPictureDisplayImage%29%29%2CvolunteeringExperiences*%28cause%2CcompanyName%2Cdescription%2CendedOn%2Crole%2CstartedOn%2Ccompany~fs_salesCompany%28entityUrn%2CpictureInfo%2Cname%2CcompanyPictureDisplayImage%29%29%2Cskills*%2Clanguages*%29'
        
        res = requests.get(
            url=f"{self.client.SALES_NAV_API_URL}/salesApiProfiles/(profileId:{profile_id},authType:name)?{decorations}", 
            cookies=self.client.session.cookies, 
            headers={
                **self.client.session.headers, 
                "x-li-track":self.client.liTrackSales, 
                "referer":f"{self.client.SALES_NAV_BASE_URL}/lead/{profile_id},name?msgType=inmail"
            }
        )
    ...
    def send_sales_inmail(self, profile_id, subject="Inmail Subject", body="Inmail Body",evade=default_evade, public_id=None):
        ...
        if public_id:
            headers["referer"]=f"{self.client.SALES_NAV_BASE_URL}/lead/{public_id},name?msgType=inmail"
